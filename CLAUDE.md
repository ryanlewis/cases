...
- Web pages have no inline scripts or styles, so the CSP holds. Everything is escaped by `html/template`. The only `template.HTML` is goldmark output, with raw HTML left off (never `html.WithUnsafe`).
- Answers from the web go through the same store functions as the CLI. Do not add another way to write events.
