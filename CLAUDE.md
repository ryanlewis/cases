...
- `internal/store/` — the store.
  ...
  - `write.go` — `Create` and one append function per event. Each append locks the case directory, loads it, runs `apply` on the new event, and only then writes the file atomically: the temp file is hard-linked to the event's name, which fails if the name is taken, and renamed only after a second check where hard links do not work. Every append function takes an optional `AtRevision(rev)`, checked in the same locked section before `apply`; a case whose `Revision()` (its number of event files) has moved on fails with `ErrStale`.
  - `store.go` — the `Store` interface the CLI and web read and write through, keyed by case id, and `Dir`, which resolves the id with `CaseDir` and calls the package functions. `Deps.Cases` and `web.New` take a `Store`; `prune`, `serve`'s instance file and `status` still use the path. Tests override one method by embedding the store (`runCasesWith`, `newAppWith`).
