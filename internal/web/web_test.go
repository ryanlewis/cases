...
func TestMarkdownCannotInjectMarkup(t *testing.T) {
	...
	for _, good := range []string{"<h1>Heading</h1>", `<a href="https://example.com">ok</a>`, "&lt;b&gt;title&lt;/b&gt;", `href="#ZgotmplZ"`} {
...
func TestMissingStoreShowsAnEmptyInbox(t *testing.T) {
	...
	w := httptest.NewRecorder()
	s.Handler().ServeHTTP(w, r)
	if w.Code != http.StatusOK || !strings.Contains(w.Body.String(), "Nothing waiting") {
