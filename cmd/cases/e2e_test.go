...
func TestDecisionEndToEnd(t *testing.T) {
	storePath := filepath.Join(t.TempDir(), "cases", "cases.db")
	...
	id := strings.TrimSpace(mustRun(t, "--store", storePath, "open",
	...
	wait := startWait(t, storePath, "--timeout", "10s")

	// The human answers from the other side of the store.
	mustRun(t, "--store", storePath, "answer", id, "--option", "1", "--note", "Revisit after 1.3")
	...
	mustRun(t, "--store", storePath, "pickup", id, "--by", "bun-pins")
	if r := runCases(t, "Pinned in #12.\n", "--store", storePath, "close", id, "--outcome-file", "-"); r.err != nil {
		t.Fatalf("close: %v", r.err)
	}

	// A closed case is no longer waited on, even from before it was opened.
	assertTimedOut(t, runCases(t, "", "--store", storePath, "wait", "--since", "2000-01-01T00:00:00Z", "--timeout", "50ms"))

	want := []string{"0001-agent-open.json", "0002-human-answer.json", "0003-agent-pickup.json", "0004-agent-close.json"}
	if names := eventFiles(t, storePath, id); !slices.Equal(names, want) {
		t.Errorf("events = %v, want %v", names, want)
	}
	if out := mustRun(t, "--store", storePath, "list", "--state", "closed"); !strings.Contains(out, id) {
