...
func TestClose(t *testing.T) {
	...
	if r := runCases(t, "", "--store", storePath, "close", id, "--outcome-file", outcome); r.err == nil {
		t.Error("closed a case that was not picked up")
	}
	mustRun(t, "--store", storePath, "pickup", id)
	if out := mustRun(t, "--store", storePath, "close", id, "--outcome-file", outcome, "--link", "https://github.com/x/y/pull/12"); out != id+" closed\n" {
	...
	if r := runCases(t, "", "--store", storePath, "close", id, "--outcome-file", filepath.Join(t.TempDir(), "missing.md")); r.err == nil {
		t.Error("missing outcome file accepted")
	}
	if r := runCases(t, "", "--store", storePath, "close", id); r.err == nil || !strings.Contains(r.err.Error(), "missing flags: --outcome=TEXT or --outcome-file=FILE") {
...
func TestCloseAtRevision(t *testing.T) {
	...
	mustRun(t, "--store", storePath, "answer", id, "--option", "1")
	mustRun(t, "--store", storePath, "pickup", id)
	refusedAsStale(t, storePath, id, 2, 3, "close", id, "--outcome", "Pinned.", "--revision", "2")
...
func TestCloseInlineOutcome(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			r := runCases(t, "", append([]string{"--store", storePath, "close", id}, tt.args...)...)
			if r.err == nil || !strings.Contains(r.err.Error(), tt.wantErr) {
				t.Errorf("err = %v, want %q", r.err, tt.wantErr)
			}
			if c := loadCase(t, storePath, id); c.Close != nil {
				t.Errorf("close = %+v", c.Close)
			}
		})
	}

	if out := mustRun(t, "--store", storePath, "close", id, "--outcome", "Acknowledged"); out != id+" closed\n" {
		t.Errorf("stdout = %q", out)
	}
	if c := loadCase(t, storePath, id); c.Close == nil || c.Close.Outcome != "Acknowledged" {
		t.Errorf("close = %+v", c.Close)
	}
}
